...
_ga.trackTwitter = function (a, c) {
    ...
    try {
        twttr && twttr.events && twttr.events.bind && twttr.events.bind("tweet", function (b) {
            if (b) {
                var c;
                b.target && "IFRAME" == b.target.nodeName && (c = _ga.extractParamFromUri_(b.target.src, "url"));
...
_ga.extractParamFromUri_ = function (a, c) {
    if (a) {
        ...
        if (1 != d.length) {
            d = decodeURI(d[1]);
            c += "\x3d";
            for (var d = d.split("\x26"), e = 0, b; b = d[e]; ++e)if (0 === b.indexOf(c))return unescape(b.split("\x3d")[1])
        }
    }
};
jQuery && jQuery("a").click(function () {
    var a = jQuery(this).attr("href");
    null != a && (a.match(/^http/i) && !a.match(document.domain) ? _gaq.push(["_trackEvent", "outgoing", "click", a]) : a.match(/\.(doc|pdf|xls|ppt|zip|txt|vsd|vxd|js|css|rar|exe|wma|mov|avi|wmv|mp3)$/i) ? _gaq.push(["_trackEvent", "download", "click", a]) : a.match(/^mailto:/i) && _gaq.push(["_trackEvent", "mailto", "click", a]))
});